...
class AreaMediaPlayersConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def _format_players_description(self, players_by_area):
        lines = []
        total_players = 0
        
        for area, players in players_by_area.items():
            lines.append(f"\n{area}:")
            for player in sorted(players, key=lambda x: x["name"]):
                lines.append(f"  • {player['name']} ({player['id']})")
                total_players += 1
                
        header = f"Media players detected by room (Total: {total_players}):"
        return header + "\n" + "\n".join(lines)
...
class AreaMediaPlayersOptionsFlow(config_entries.OptionsFlow):
    ...
    _format_players_description = AreaMediaPlayersConfigFlow._format_players_description
